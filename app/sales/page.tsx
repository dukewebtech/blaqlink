      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Comprehensive sales analytics and insights</p>
          </div>
            ...
            <div className="space-y-4">
              {productSales.slice(0, 10).map((product, index) => (
                <div
                  ...
                  className="flex items-center gap-3 p-3 sm:p-4 bg-muted/30 rounded-lg"
                >
                  <div className="flex items-center justify-center w-8 h-8 rounded-full bg-primary/10 text-primary font-semibold shrink-0 text-sm">
                    {index + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      <span className="text-xs text-muted-foreground">{product.total_quantity} sold</span>
                    </div>
                  </div>
                  <p className="text-sm sm:text-base font-bold shrink-0">{formatCurrency(product.total_revenue)}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {orders.length === 0 ? (
            ...
            <div className="space-y-3">
              {orders.slice(0, 10).map((order) => (
                <div key={order.id} className="flex items-start justify-between gap-3 p-3 sm:p-4 bg-muted/30 rounded-lg">
                  ...
                  <div className="text-right space-y-1 shrink-0">
                    <p className="text-sm sm:text-base font-bold">{formatCurrency(order.total_amount)}</p>
                    <Badge className="bg-green-100 text-green-800 text-xs">{order.payment_status}</Badge>
                  </div>
                </div>
              ))}
            </div>
