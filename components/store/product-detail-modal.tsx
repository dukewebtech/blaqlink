...
export function ProductDetailModal({
  ...
  const isPhysicalProduct = product.product_type === "physical"
  ...
  if (isPhysicalProduct) {
    const shippingLocations = product.shipping_locations || []
    const hasShippingLocations = shippingLocations.length > 0
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
            ...
            <div className="p-8 space-y-6">
                  ...
                  <div className="space-y-2">
                    {shippingLocations.map((location, index) => (
                      <div key={index} className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-semibold">₦{Number.parseFloat(location.price).toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                  <p className="text-xs text-gray-500 mt-2">Estimated delivery: 2-5 business days</p>
                </div>
              )}
              ...
              <div className="border-t pt-6 space-y-4">
                ...
                {product.is_automated_delivery && (
                  <p className="text-xs text-center text-gray-500">
                    Automated delivery will be processed after purchase
                  </p>
                )}
              </div>
            </div>
